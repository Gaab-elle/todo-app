...
<head>
    ...
    <title>{{ __('messages.register') }} - {{ config('app.name', 'Dev Project Manager') }}</title>
...
<body class="min-h-screen gradient-bg flex items-center justify-center p-4">
    <div class="w-full max-w-md">
        ...
        <div class="text-center mb-8 animate-fade-in">
            ...
            <p class="text-white/80">{{ __('messages.create_your_account') }}</p>
        </div>

        <!-- Register Form -->
        <div class="bg-white/10 glass-effect rounded-2xl p-8 shadow-2xl animate-slide-up">
            <h2 class="text-2xl font-bold text-white text-center mb-6">{{ __('messages.register') }}</h2>
            ...
            <div class="space-y-3">
                ...
                <a href="{{ route('auth.github') }}" 
                   class="w-full flex items-center justify-center px-4 py-3 bg-white/10 hover:bg-white/20 border border-white/30 rounded-lg text-white font-medium transition-all duration-300 hover:scale-105 transform">
                    ...
                    {{ __('messages.register_with_github') }}
                </a>
                @endif
            </div>
            @else
